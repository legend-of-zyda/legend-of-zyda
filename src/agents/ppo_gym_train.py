########################################################################################################
## PPO training code compatible with Lux Open AI Gym Interface originating from the interface's
## tutorial here https://github.com/glmcdona/LuxPythonEnvGym
## To run this code, you must install the interface and then run this file as specified by the
...
def get_command_line_arguments():
    ...
    parser.add_argument('--id', help='Identifier of this run', type=str, default=str(random.randint(0, 10000))) # str(random.randint(0, 10000))
    parser.add_argument('--learning_rate', help='Learning rate', type=float, default=0.001) # 0.001
    parser.add_argument('--gamma', help='Gamma', type=float, default=0.995) # 0.995
    parser.add_argument('--gae_lambda', help='GAE Lambda', type=float, default=0.95) # 0.95
    parser.add_argument('--batch_size', help='batch_size', type=int, default=2048)  # 2048
    parser.add_argument('--step_count', help='Total number of steps to train', type=int, default=10000000) # 10000000
    parser.add_argument('--n_steps', help='Number of experiences to gather before each learning period', type=int, default=2048) #2048
    parser.add_argument('--path', help='Path to a checkpoint to load to resume training', type=str, default=None)
    # parser.add_argument('--n_envs', help='Number of parallel environments to use in training', type=int, default=1) # 1
...
def train(args):
    ...
    if args.path:
        ...
        model = PPO("MlpPolicy",
                    ...
                    ) ## THIS LINE MAY BE REPLACED WITH ANY OTHER STABLE BASELINES MODEL (A2C, DDPG, DQN, SAC) however, parameters must be changed
    ...
    ## MULTI ENVIRONMENT SETUPS NOT CURRENTLY USED
    ...
    ################################################################################
    ## SELF PLAY AFTER INITIAL TRAINING - Default parameters: lr = something smaller
    ## than above, gamma = 0.997, gae_l = 0.95, timesteps = 200
    ...
    model.save(path=f'models/FINAL_SELFPLAY_MODEL.zip')
    env.close()
    print("Done")
